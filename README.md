...
MCSH运行的服务器目前只能实现**局域网联机**，**但可以通过端口映射来进行公网联机**。
    
登录路由器的后台（管理界面），找到并点击*端口映射（或虚拟服务器等）*，IP栏填写本机IP，内部端口和外部端口都填写`25565`（若进行过更改就填写更改后的端口），协议选择`TCP`，点击`保存`。
